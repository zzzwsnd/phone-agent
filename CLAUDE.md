...
语音管道提供商：**火山引擎（Volcengine）**
- **STT**: 火山引擎语音识别（`livekit-plugins-volcengine`）
- **TTS**: 火山引擎语音合成（`livekit-plugins-volcengine`，音色 BV001_V2_streaming）
- **LLM**: 火山方舟 GLM-5.1（通过 OpenAI 兼容接口接入，`livekit-plugins-openai` + ARK base_url）

### 核心文件

- **`agents/llm_agent.py`** — 主入口，包含 InboundAgent 类、inbound_entrypoint 函数、build_agent_session 语音管道构建、extract_caller_number 工具函数
...
- **`config/agent_config.py`** — LLM 模型配置（支持 DeepSeek/Doubao/Qwen/OpenAI，通过 `MODEL_PROVIDER` 环境变量切换，历史遗留未使用）
- **`config/livekit_config.py`** — LiveKit、SIP 连接配置和火山引擎（STT/TTS/LLM）配置
...
→ 创建 InboundAgent（prompt 含回访摘要）→ AgentSession 启动（火山引擎 STT/TTS/LLM）
...
4. 校验火山引擎必填配置（STT/TTS/LLM），缺失则跳过本次 Job
5. 创建 InboundAgent（prompt 注入回访摘要），通过 `build_agent_session()` 构建火山引擎语音管道
6. LLM 在 3 轮内采集访客信息，调用 save_visitor_record 保存
7. 保存后推送微信通知给门卫，结束通话

### LLM 配置

AgentSession 使用 `OpenAI(model="glm-5.1", base_url="https://ark.cn-beijing.volces.com/api/v3", api_key=VOLCENGINE_LLM_API_KEY, temperature=0.7)`，通过火山方舟 ARK 平台的 OpenAI 兼容接口接入。`config/agent_config.py` 导出的 LLM 实例（`supervisor_llm`/`fast_llm`/`reasoning_llm`）为历史遗留，当前未被使用。
...
**火山引擎 STT/TTS**：
- `VOLCENGINE_STT_APP_ID` — STT 应用 ID
- `VOLCENGINE_STT_CLUSTER` — STT 集群（默认 volcengine_streaming_common）
- `VOLCENGINE_TTS_APP_ID` — TTS 应用 ID
- `VOLCENGINE_TTS_CLUSTER` — TTS 集群（默认 volcano_tts）
- `VOLCENGINE_ACCESS_TOKEN` — 火山引擎访问令牌（STT/TTS 共用）

**火山方舟 LLM**：
- `VOLCENGINE_LLM_API_KEY` — 火山方舟 API Key
- `VOLCENGINE_LLM_MODEL` — 模型名称（默认 glm-5.1）
- `VOLCENGINE_LLM_BASE_URL` — ARK 平台 API 地址（默认 https://ark.cn-beijing.volces.com/api/v3）
...
**模型**：
- `MODEL_PROVIDER`（默认 deepseek）、对应提供商的 API Key（历史遗留，当前未使用）
